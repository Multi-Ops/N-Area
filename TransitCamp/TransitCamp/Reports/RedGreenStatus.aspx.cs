...
{
    public partial class RedGreenStatus : System.Web.UI.Page
    {
        ...
        protected void BindCity()
        {
            ...
            var data = aDTypeServices.GetADTypeDetails();
            ddlStatus.DataSource = data;
            ddlStatus.DataValueField = "ID";
            ddlStatus.DataTextField = "ADTypeName";
            ddlStatus.DataBind();
        ...
        protected void Bind(DateTime from, DateTime to, Int64 id, int cid, string type)
        {
            ...
            var det = aDTypeServices.GetADTypeDetails();
            var data = aDServices.StatusWiseReport(from, to, id, cid);
            if (type.ToLower().Trim() == "all")
            {
                if (det != null)
                {
                    foreach (var res in det)
                    {
                        ...
                        var finaldata = data.OrderBy(x => x.ADTypeName).ToList();
        ...
        //    var data = aDServices.StatusWiseReportID(id, finaldate);
        //    foreach (var r in data)
        //    {
        //        if (r.MoveID != null)
        //        {
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    if (e.Row.DataItem as DataRowView != null)
                    {
                        ...
                        String Id = Convert.ToString(dt.Rows[e.Row.RowIndex]["ADTypeName"]);
        ...
        protected void btnSearchAD_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (hfFromDate.Value == "" && hfToDate.Value == "" && ddlStatus.SelectedValue != "")
                    {
                        Int64 id = Convert.ToInt64(ddlStatus.SelectedValue);
                        //Bind(id);
                    }
                    else
                    {
                        ...
                        {
                            Bind(fromdate, todate, id, Convert.ToInt32(ddlCities.SelectedValue), ddlStatus.SelectedItem.Text);
        ...
        {
            Response.Redirect("RedGreenStatusDateWise");
        }
    }
}