...
{
    public partial class OnHoldingDateRangeReport : System.Web.UI.Page
    {
        ...
        protected void Bind()
        {
            ...
            var data = aDServices.PagingOnHold();
            //var data = aDServices.GetByDateForHoldingReport(finaldate);
        ...
        protected void BindSearch(DateTime fromdate, DateTime todate, int cid)
        {
            #region declare
            int lve = 0;
            int td = 0;
            ...
            string fromDay = FromDate.Day.ToString();
            string fromMonth = FromDate.Month.ToString();
            string fromYear = FromDate.Year.ToString();

            DateTime ToDate = todate;
            string toDay = ToDate.Day.ToString();
            string toMonth = ToDate.Month.ToString();
            string toYear = ToDate.Year.ToString();

            string finalstringfrom = fromYear + "-" + fromMonth + "-" + fromDay;
            string finalstringto = toYear + "-" + toMonth + "-" + toDay;

            DateTime fromdatefinal = DateTime.Parse(finalstringfrom);
            DateTime todatefinal = DateTime.Parse(finalstringto);

            var data = aDServices.GetByDateForOnHoldingReportDateRange(fromdatefinal, todatefinal, cid);
            if (data != null)
            {
                #region lve
            ...
            btnPrint.Visible = true;
            PrintDiv.Visible = true;
            lblDate.Text = fromdatefinal.ToString("dd-MM-yyyy");
            lblTODate.Text = todatefinal.ToString("dd-MM-yyyy");
            grd.DataSource = dt;
            grd.DataBind();
        }
        ...
        private void AddTotalRow(string cat)
        {
            ...
            row.Cells.AddRange(new TableCell[10] { new TableCell { Text = cat, HorizontalAlign = HorizontalAlign.Left},
                                        new TableCell (),
                                        new TableCell (),
                                        new TableCell (),
                                        new TableCell (),
        ...
        protected void btnSearchAD_Click1(object sender, EventArgs e)
        {
            ...
            if (hfFromDate.Value != "" && hfToDate.Value != "")
            {
                ...
                BindSearch(selecteddateFrom, selecteddateTo, Convert.ToInt32(ddlCities.SelectedValue));
                hfFromDate.Value = "";
                hfToDate.Value = "";
            }
            else
        ...
        {
            Response.Redirect("OnHoldingReport");
        }
    }
}