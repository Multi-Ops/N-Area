...
{
    public partial class ADReportDateWise : System.Web.UI.Page
    {
        ...
        protected void Bind()
        {
            ...
            lblFrom.Text = today.ToString("dd MMMM yyyy");
            lblTo.Text = today.ToString("dd MMMM yyyy");
        }

        protected void btnSearchdate_Click(object sender, EventArgs e)
        {
            if (hfFromDate.Value.ToString() == "" || hfToDate.Value.ToString() == "")
            {
                btnPrint.Visible = false;
                ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Select Dates!');", true);
            }
            else
            {
                DateTime fromdate = Convert.ToDateTime(Helper.CommonFunctions.ConvertToCreatedOnDate(hfFromDate.Value));
                string finalFrom = fromdate.Year.ToString() + "-" + fromdate.Month.ToString() + "-" + fromdate.Day.ToString();

                DateTime todate = Convert.ToDateTime(Helper.CommonFunctions.ConvertToCreatedOnDate(hfToDate.Value));
                string finalTo = todate.Year.ToString() + "-" + todate.Month.ToString() + "-" + todate.Day.ToString();

                if (todate < fromdate)
                {
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Select Valid From-To Date!');", true);
                }
                else
                {
                    ...
                    DateTime today = DateTime.Now;
                    var data = aDServices.GetByDate(Convert.ToDateTime(finalFrom), Convert.ToDateTime(finalTo), Convert.ToInt32(ddlCities.SelectedValue));
                    ...
                    if (data != null)
                    {
                        ...
                        foreach (var r in data)
                        {
                            ...
                            if (r.IsManifest == true)
                            {
                                manifestServices = new ManifestServices(new TCContext());
                                var manData = manifestServices.GetByADID(r.ID);

                                if (manData != 0)
                                {
                                    var fManData = manifestServices.GetByID(manData);
                                    r.DepDate = fManData.ManifestDate;
                                }
                            }
                            else if (r.IsTempHold == true)
                            {
                                r.DepDate = r.UpdatedOn;
                            }
                        }
                    }
                    ...
                    grd.DataBind();
                    btnPrint.Visible = true;
                    txtFrom.Text = fromdate.ToString("dd MMMM yyyy");
                    txtTo.Text = todate.ToString("dd MMMM yyyy");
        ...
        {
            Response.Redirect("ADReportToday");
        }
    }
}