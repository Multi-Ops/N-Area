...
{
    public partial class DepartureDateWise : System.Web.UI.Page
    {
        ...
        protected void BindCities()
        {
            ...
            ddlCities.DataBind();
            ddlCities.Items.Insert(0, new ListItem("-- Select --", ""));
            //ddlCities.Items.Insert(1, new ListItem("All", "0"));
        }

        protected void Bind(DateTime from, DateTime to, Int64 cityId)
        {
            #region declare
            int lve = 0;
            int td = 0;
            ...
            var data = aDServices.DepartureDateWiseReport(from, to, cityId);
            if (data != null)
            {
                #region lve
            ...
            hfFromDate.Value = from.ToString("dd-MM-yyyy hh:mm:ss");
            hfToDate.Value = to.ToString("dd-MM-yyyy hh:mm:ss");
        ...
        protected void Bind()
        {
            ...
            var data = aDServices.DepartureDateWiseReport(finaldate);
            if (data != null)
            {
                #region lve
                var lveOfficer = (from p in data
            ...
            hfFromDate.Value = finaldate.ToString("dd-MM-yyyy hh:mm:ss");
            hfToDate.Value = finaldate.ToString("dd-MM-yyyy hh:mm:ss");
            txtTo.Text = finaldate.ToString("dd-MM-yyyy");
            grd.DataSource = dt;
            grd.DataBind();
        }
        ...
        protected void grd_RowCreated(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    if (Convert.ToString(ddlCities.SelectedIndex) == "1")
                    {
                        if (e.Row.DataItem as DataRowView != null)
                        {
                            DataTable dt = (e.Row.DataItem as DataRowView).DataView.Table;
                            String Id = Convert.ToString(dt.Rows[e.Row.RowIndex]["CityName"]);
        ...
        protected void btnSearchAD_Click(object sender, EventArgs e)
        {
            ...
            {
                lblError.Text = "Select From-To Date.";
            ...
            else
            {
                ...
                else if (ddlCities.SelectedValue.ToString() == "")
                {
                    lblError.Text = "Select Drop Down Date.";
                    lblError.Visible = true;
                }
                else
                {
                    Int64 cID = Convert.ToInt64(ddlCities.SelectedValue);
                    Bind(fromdate, todate, cID);
                    if (ddlCities.SelectedItem.ToString() != string.Empty)
        ...
        {
            //Bind();
            Response.Redirect("DepartureDateWiseDaily");
        }
    }
}