...
namespace TransitCamp.Reports
{
    public partial class WaitingReport : System.Web.UI.Page
    {
        ...
        protected void BindAll(DateTime from, DateTime to)
        {
            int lve = 0;
            int td = 0;
            int posting = 0;
            ...
            var cities = cityServices.GetCityDetails();
            if (cities != null)
            {
                foreach (var cityRes in cities)
                {
                    //DataRow dr1 = dt.NewRow();
                    //dr1["ADNO"] = cityRes.CityName;
                    //dt.Rows.Add(dr1);

                    var data = aDServices.WaitingListReport(cityRes.ID, from, to);
                ...
                lblDiv.Text = ddlCities.SelectedItem.ToString();
                grd.DataSource = dt;
                grd.DataBind();
            }
        }
        ...
        protected void Bind(DateTime from, DateTime to, Int64 id)
        {
            ...
            var data = aDServices.WaitingListReport(id, from, to);
            #region lve
            var lveOfficer = (from p in data
                              where p.CategoryName.ToLower().Contains("officer") && p.MoveName.ToLower() == "lve"
                              select p).ToList();
        ...
        protected void btnSearchAD_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    if (hfFromDate.Value == "" && hfToDate.Value == "" && ddlCities.SelectedValue != "")
                    {
                        Int64 id = Convert.ToInt64(ddlCities.SelectedValue);
                        Bind(id);
                    }
                    else if (ddlCities.SelectedItem.Text.ToLower().Trim() == "all")
                    {
                        Int64 id = Convert.ToInt64(ddlCities.SelectedValue);
