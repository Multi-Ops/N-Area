...
{
    public partial class DepartureMoveTypeDateWise : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                BindMoveType();
                Int64 id = Convert.ToInt64(ddlMoveType.SelectedValue);
                Bind(id);
                BindCities();
            }
        }
        ...
        protected void BindMoveType()
        {
            moveServices = new MoveServices(new TCContext());
            var data = moveServices.GetMoveDetails();
            ddlMoveType.DataSource = data;
            ddlMoveType.DataValueField = "ID";
            ddlMoveType.DataTextField = "MoveName";
            ddlMoveType.DataBind();
            ddlMoveType.Items.Insert(0, new ListItem("All", "0"));
        }
        ...
        protected void Bind(Int64 id)
        {
            ...
            var data = aDServices.DepartureMoveTypeWiseReportID(id, finaldate);
            ...
            lblMoveType.Text = ddlMoveType.SelectedItem.ToString();
            grd.DataSource = dt;
            grd.DataBind();
        }

        protected void Bind(Int64 id, DateTime date, int cid, string type)
        {
            #region declare
            int lve = 0;
            int td = 0;
            ...
            var det = moveServices.GetMoveDetails();
            var data = aDServices.DepartureMoveTypeWiseReportID(id, finaldate, cid);
                    ...
                    foreach (var res in det)
                    {
                        if (data != null)
                        {
                            foreach (var r in data)
                            {
                                ...
                                dr["MoveName"] = res.MoveName;
                                dt.Rows.Add(dr);
                            }
                        }

                        var finaldata = data.OrderBy(x => x.MoveName).ToList();
        ...
        protected void grd_RowCreated(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    if (Convert.ToString(ddlMoveType.SelectedItem.Text.Trim().ToLower()) == "all")
                    {
                        if (e.Row.DataItem as DataRowView != null)
                        {
                            DataTable dt = (e.Row.DataItem as DataRowView).DataView.Table;
                            String Id = Convert.ToString(dt.Rows[e.Row.RowIndex]["MoveName"]);
        ...
        protected void btnSearchAD_Click(object sender, EventArgs e)
        {
            ...
            {
                if (hfFromDate.Value == "" && ddlMoveType.SelectedValue != "")
                {
                    Int64 id = Convert.ToInt64(ddlMoveType.SelectedValue);
                    Bind(id);
                }
                else
                {
                    ...
                    Bind(id, fromdate, Convert.ToInt32(ddlCities.SelectedValue), ddlMoveType.SelectedItem.Text);
        ...
        {
            Response.Redirect("DepartureMoveTypeDateWise");
        }
    }
}