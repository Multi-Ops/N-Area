...
{
    public partial class WaitingDateWise : System.Web.UI.Page
    {
        ...
        protected void BindAll(DateTime from)
        {
            int lve = 0;
            int td = 0;
            int posting = 0;
            ...
            var data = aDServices.WaitingListReportMod(from);
        ...
        protected void Bind(DateTime from, Int64 id)
        {
            int lve = 0;
            int td = 0;
            int posting = 0;
            ...
            var data = aDServices.WaitingListReportMod(id, from);
            #region lve
            var lveOfficer = (from p in data
                              where p.CategoryName.ToLower().Contains("officer") && p.MoveName.ToLower() == "lve"
                              select p).ToList();
