...
{
    public partial class DepartureUnitWiseReportDateWise : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                BindUnit();
                Int64 id = Convert.ToInt64(ddlUnit.SelectedValue);
                Bind(id);
                BindCities();
        ...
        protected void Bind(Int64 id)
        {
            ...
            var data = aDServices.DepartureUnitWiseReportID(id, finaldate);
            ...
            lblUnit.Text = ddlUnit.SelectedItem.ToString();
            grd.DataSource = dt;
            grd.DataBind();
        }
        ...
        protected void Bind(Int64 id, DateTime date, int cid, string type)
        {
            ...
            var det = unitServices.GetUnitDetails();
            var data = aDServices.DepartureUnitWiseReportID(id, finaldate, cid);
                    ...
                    foreach (var res in det)
                    {
                        if (data != null)
                        {
                            foreach (var r in data)
                            {
                                ...
                                dr["UnitName"] = res.UnitName;
                                dt.Rows.Add(dr);
                            }
                        }

                        var finaldata = data.OrderBy(x => x.UnitName).ToList();
        ...
        protected void grd_RowCreated(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    if (Convert.ToString(ddlUnit.SelectedItem.Text.Trim().ToLower()) == "all")
                    {
        ...
        protected void btnSearchAD_Click(object sender, EventArgs e)
        {
            ...
            {
                if (hfFromDate.Value == "" && ddlUnit.SelectedValue != "")
                {
                    Int64 id = Convert.ToInt64(ddlUnit.SelectedValue);
                    Bind(id);
                }
                else
                {
                    ...
                    Bind(id, fromdate, Convert.ToInt32(ddlCities.SelectedValue), ddlUnit.SelectedItem.Text);
