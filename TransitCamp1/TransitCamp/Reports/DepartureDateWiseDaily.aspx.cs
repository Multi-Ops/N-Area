...
{
    public partial class DepartureDateWiseDaily : System.Web.UI.Page
    {
        ...
        protected void Bind(DateTime date, int cityID)
        {
            #region declare
            int lve = 0;
            int td = 0;
            ...
            var data = aDServices.DepartureDateWiseReport(finaldate, cityID);
            if (data != null)
            {
                #region lve
                var lveOfficer = (from p in data
        ...
        protected void btnSearchAD_Click(object sender, EventArgs e)
        {
            ...
            if (ddlCities.SelectedValue.ToString() == string.Empty)
            {
                lblCity.Visible = true;
                lblCity.Text = "Select City From Dropdown";
            }
            else if (hfFromDate.Value == "")
            {
                Bind();
            }
            else
