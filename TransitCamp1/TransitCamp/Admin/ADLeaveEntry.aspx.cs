...
{
    public partial class ADLeaveEntry : System.Web.UI.Page
    {
        protected ICityServices cityServices;
        protected IADLeaveServices adLeaveServices;
        ...
        {
            Int32 id = Convert.ToInt32(Request.QueryString["ADLID"]);
            adLeaveServices = new ADLeaveServices(new TCContext());
            var getdetails = adLeaveServices.GetByID(id);
            if (getdetails != null)
            {
        ...
        protected void InsertUpdate()
        {
            ...
            else
            {
                adLeaveServices = new ADLeaveServices(new TCContext());
                Int32 id = Convert.ToInt32(Request.QueryString["ADLID"]);
                BusinessLayer.ADLeaveEntry info = new BusinessLayer.ADLeaveEntry();
                ...
                if (id != 0)
                {
                    info.ID = Convert.ToInt32(Request.QueryString["ADLID"]);
                    info.UpdatedOn = DateTime.Now;
                    adLeaveServices.Update(info);
                    adLeaveServices.Save();
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Data Updated!');window.location = 'ADLeaveList';", true);
                }
                else
                {
                    info.CreatedOn = DateTime.Now;
                    adLeaveServices.Insert(info);
                    adLeaveServices.Save();
                    ScriptManager.RegisterStartupScript(this, GetType(), "showalert", "alert('Data Saved!');window.location = 'ADLeaveList';", true);
                }
            }
        }
